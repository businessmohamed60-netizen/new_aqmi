...
class AssessmentController
{
    ...
    public function saveLead(): void
    {
        ...
        // NOTE: aucun Report n'est créé ici. Les résultats sont consultables
        // gratuitement dès maintenant ; une demande de certification (et donc
        // une ligne dans `reports`) n'est créée que si l'utilisateur clique
        // explicitement sur "Demander un Rapport AQMI Certifié" (voir
        // requestCertification() ci-dessous).
    ...
    /**
     * Déclenché uniquement par le clic explicite de l'utilisateur sur
     * "Demander un Rapport AQMI Certifié". C'est ici, et seulement ici,
     * qu'une ligne `reports` est créée.
     */
    public function requestCertification(array $params): void
    {
        $assessmentId = (int)($params['id'] ?? 0);
        $assessment = Assessment::find($assessmentId);
        if (!$assessment || $assessment['status'] !== 'completed') { redirect('/'); return; }
        ...
        // Empêche une double demande tant qu'une demande est déjà ouverte ou traitée avec succès
        $existing = \App\Models\Report::findByAssessment($assessmentId);
        if ($existing && in_array($existing['status'], ['certification_requested', 'under_review', 'approved', 'certified'], true)) {
            $_SESSION['success'] = 'Une demande de certification est déjà en cours pour cette évaluation.';
            redirect('/assessment/' . $assessmentId . '/results');
            return;
        }
        ...
        try {
            ...
            // La demande reste enregistrée même si l'email de notification échoue
        }

        $_SESSION['success'] = 'Votre demande de Rapport AQMI Certifié a été envoyée. Statut : en attente de validation.';
        redirect('/assessment/' . $assessmentId . '/results');
    }
