...
(function () {
  ...
  const DEFAULT_CONFIGS = {
    ...
    cover_page:      { company_name: 'Nom de l\'entreprise', report_title: 'Rapport d\'Audit Qualité', subtitle: 'Automotive Quality Maturity Index', show_logo: true, show_stamp: true, show_date: true, show_number: true, accent_color: '#0f2845', gold_color: '#c5a455', bg_color: '#faf8f3', text_color: '#0f2845', font_size_title: '1.5rem', font_size_subtitle: '0.9rem', padding: '40px', border_color: '#EEF2F7', bg_image_url: '', bg_image_opacity: 1, bg_image_size: 'cover', bg_image_position: 'center', bg_image_repeat: 'no-repeat', cert_body_text: 'Ce document atteste que l\'entreprise mentionnée ci-dessous a fait l\'objet d\'une évaluation complète de sa maturité qualité selon le référentiel AQMI.', signature_label: 'Direction Qualité AQMI', signature_role: 'Auditeur Principal', show_expiry: false },
  ...
  function BlockPreview({ blockKey, cfg }) {
    ...
    switch (blockKey) {
      ...
      case 'cover_page': {
        const gold = cfg.gold_color || '#c5a455';
        const accent = cfg.accent_color || '#0f2845';
        const cpBgStyle = { position: 'relative', minHeight: '380px', backgroundColor: cfg.bg_color || '#faf8f3', borderRadius: '2px', overflow: 'hidden', padding: '0' };
        if (cfg.bg_image_url) {
        ...
        return h('div', { className: 'rs-block-cover-page cert-premium', style: cpBgStyle },
          h('div', { style: { position: 'absolute', inset: '8px', border: '2.5px solid ' + gold, borderRadius: '1px', pointerEvents: 'none', zIndex: 1 } }),
          h('div', { style: { position: 'absolute', inset: '11px', border: '1.5px solid ' + gold, opacity: 0.55, borderRadius: '1px', pointerEvents: 'none', zIndex: 1 } }),
          h('div', { style: { position: 'absolute', inset: '14px', border: '1px solid ' + gold, opacity: 0.45, borderRadius: '1px', pointerEvents: 'none', zIndex: 1 } }),
          ['tl','tr','bl','br'].map(function(c){ return h('div', { key: 'flourish-'+c, style: { position: 'absolute', width: '48px', height: '48px', zIndex: 3, pointerEvents: 'none', top: c.indexOf('t')>=0?'14px':'auto', bottom: c.indexOf('b')>=0?'14px':'auto', left: c.indexOf('l')>=0?'14px':'auto', right: c.indexOf('r')>=0?'14px':'auto' } }); }),
          h('div', { style: { position: 'absolute', left: '30px', right: '30px', height: '1.5px', top: '22px', background: 'linear-gradient(90deg, transparent, ' + gold + ', transparent)', zIndex: 2 } }),
          h('div', { style: { position: 'absolute', left: '30px', right: '30px', height: '1.5px', bottom: '22px', background: 'linear-gradient(90deg, transparent, ' + gold + ', transparent)', zIndex: 2 } }),
          h('div', { style: { position: 'relative', zIndex: 5, minHeight: '360px', display: 'flex', flexDirection: 'column', justifyContent: 'space-between', padding: '36px 30px' } },
            h('div', { className: 'd-flex justify-content-between align-items-start' },
              cfg.show_logo ? h('div', null,
                h('span', { style: { fontWeight: '800', letterSpacing: '6px', fontSize: '1.7rem', color: accent, display: 'inline-flex', alignItems: 'center', gap: '12px', lineHeight: '1' } }, 'AQMI'),
                h('div', { style: { fontSize: '0.65rem', color: '#8298b5', letterSpacing: '0.18em', textTransform: 'uppercase', fontWeight: '500', marginTop: '6px' } }, 'Automotive Quality Maturity Index')) : null,
              h('div', { className: 'text-end' },
                cfg.show_number ? h('div', null,
                  h('div', { style: { fontSize: '0.58rem', letterSpacing: '0.12em', textTransform: 'uppercase', fontWeight: '600', color: '#8298b5' } }, 'Référence'),
                  h('div', { style: { fontSize: '0.78rem', fontWeight: '700', color: accent, fontFamily: 'monospace' } }, 'AQMI-RPT-000')) : null,
                cfg.show_date ? h('div', { style: { fontSize: '0.68rem', color: '#5a6a82', marginTop: '4px' } }, h('i', { className: 'bi bi-calendar3', style: { fontSize: '0.65rem' } }), ' ' + new Date().toLocaleDateString('fr-FR')) : null)),
            h('div', { className: 'text-center', style: { flexGrow: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', padding: '24px 0' } },
              h('div', { style: { display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '10px', margin: '14px 0' } },
                h('span', { style: { display: 'block', width: '100px', height: '1px', background: 'linear-gradient(90deg, transparent, ' + gold + ', transparent)' } }),
                h('span', { style: { display: 'inline-block', width: '8px', height: '8px', background: gold, transform: 'rotate(45deg)', flexShrink: 0, opacity: 0.85, boxShadow: '0 0 4px ' + gold } }),
                h('span', { style: { display: 'block', width: '100px', height: '1px', background: 'linear-gradient(90deg, transparent, ' + gold + ', transparent)' } })),
              h('h1', { style: { fontFamily: 'Georgia, serif', fontSize: '1.85rem', fontWeight: '700', color: accent, textTransform: 'uppercase', letterSpacing: '0.04em', margin: '0', textShadow: '0 1px 2px rgba(15,40,69,0.06)' } }, esc(cfg.report_title || 'Rapport')),
              h('p', { style: { fontSize: '0.85rem', color: '#5a6a82', fontWeight: '500', margin: '6px 0 0', letterSpacing: '0.04em', textTransform: 'uppercase' } }, esc(cfg.subtitle || '')),
              h('div', { style: { display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '10px', margin: '14px 0' } },
                h('span', { style: { display: 'block', width: '100px', height: '1px', background: 'linear-gradient(90deg, transparent, ' + gold + ', transparent)' } }),
                h('span', { style: { display: 'inline-block', width: '8px', height: '8px', background: gold, transform: 'rotate(45deg)', flexShrink: 0, opacity: 0.85, boxShadow: '0 0 4px ' + gold } }),
                h('span', { style: { display: 'block', width: '100px', height: '1px', background: 'linear-gradient(90deg, transparent, ' + gold + ', transparent)' } })),
              h('div', { style: { margin: '20px 0 12px' } },
                h('p', { style: { fontSize: '0.65rem', letterSpacing: '0.14em', textTransform: 'uppercase', fontWeight: '600', color: '#8298b5', margin: '0 0 4px' } }, 'Rapport préparé pour'),
                h('h2', { style: { fontFamily: 'Georgia, serif', fontSize: '1.55rem', fontWeight: '700', color: accent, margin: '0', letterSpacing: '0.01em', textShadow: '0 1px 2px rgba(15,40,69,0.05)' } }, esc(cfg.company_name || ''))),
              cfg.cert_body_text ? h('p', { style: { fontSize: '0.78rem', color: '#5a6a82', lineHeight: '1.7', maxWidth: '400px', margin: '14px auto', fontStyle: 'italic' } }, esc(cfg.cert_body_text)) : null,
              cfg.show_stamp ? h('div', { style: { margin: '20px 0 8px', display: 'flex', justifyContent: 'center' } },
                h('div', { style: { width: '100px', height: '100px', borderRadius: '50%', border: '2.5px solid ' + gold, display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative', boxShadow: '0 2px 12px rgba(197,164,85,0.15)', background: 'radial-gradient(circle at 50% 50%, rgba(255,255,255,0.6) 0%, rgba(250,248,243,0.3) 100%)' } },
                  h('div', { style: { position: 'absolute', inset: '6px', border: '1.5px solid ' + gold, borderRadius: '50%', opacity: 0.35 } }),
                  h('div', { style: { position: 'absolute', inset: '10px', border: '0.5px solid ' + gold, borderRadius: '50%', opacity: 0.2 } }),
                  h('div', { style: { display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '1px', zIndex: 2 } },
                    h('i', { className: 'bi bi-patch-check-fill', style: { fontSize: '1.35rem', color: gold, marginBottom: '2px' } }),
                    h('span', { style: { fontWeight: '800', fontSize: '0.7rem', letterSpacing: '1.5px', color: accent } }, 'CERTIFIÉ'),
                    h('span', { style: { fontSize: '0.55rem', letterSpacing: '2px', opacity: 0.7, fontWeight: '600', color: accent } }, 'AQMI')),
                  h('div', { style: { position: 'absolute', bottom: '10px', display: 'flex', gap: '4px', fontSize: '0.5rem', letterSpacing: '2px', color: gold } },
                    h('span', null, '★'), h('span', null, '★'), h('span', null, '★')))) : null),
            h('div', { style: { marginTop: '16px' } },
              h('div', { style: { height: '1px', marginBottom: '16px', background: 'linear-gradient(90deg, transparent, ' + gold + ', transparent)' } }),
              h('div', { className: 'd-flex justify-content-between align-items-end' },
                h('div', { style: { maxWidth: '200px' } },
                  h('div', { style: { borderTop: '1.5px solid ' + accent, width: '180px', marginBottom: '6px' } }),
                  h('p', { style: { fontSize: '0.74rem', fontWeight: '700', color: accent, margin: '0', letterSpacing: '0.02em' } }, esc(cfg.signature_label || 'Direction Qualité AQMI')),
                  h('p', { style: { fontSize: '0.65rem', color: '#8298b5', margin: '2px 0 0' } }, esc(cfg.signature_role || 'Auditeur Principal'))),
                cfg.show_expiry ? h('div', { className: 'text-center' },
                  h('span', { style: { display: 'block', fontSize: '0.58rem', letterSpacing: '0.1em', textTransform: 'uppercase', color: '#8298b5', fontWeight: '600' } }, 'Valide jusqu\'au'),
                  h('span', { style: { display: 'block', fontSize: '0.72rem', fontWeight: '700', color: accent, marginTop: '2px' } }, '—')) : null,
                h('div', { className: 'text-end' },
                  h('span', { style: { display: 'block', fontSize: '0.58rem', letterSpacing: '0.12em', textTransform: 'uppercase', color: '#8298b5', fontWeight: '600' } }, 'AQMI · NOVAQYS'),
                  h('span', { style: { display: 'block', fontSize: '0.55rem', color: '#a0afbf', marginTop: '2px' } }, 'Document confidentiel')))));
      }
      case 'kpi_card':
    ...
    switch (blockKey) {
      ...
      case 'cover_page':
        return h(React.Fragment, null,
          ...
          colorField('Couleur accent', 'accent_color', val('accent_color', '#0f2845'), setCfg),
          colorField('Couleur or', 'gold_color', val('gold_color', '#c5a455'), setCfg),
          colorField('Couleur fond', 'bg_color', val('bg_color', '#faf8f3'), setCfg),
          ...
          field('Texte certificat', 'text', 'cert_body_text', val('cert_body_text', ''), setCfg),
          field('Label signature', 'text', 'signature_label', val('signature_label', ''), setCfg),
          field('Rôle signature', 'text', 'signature_role', val('signature_role', ''), setCfg),
          ...
          switchField('Date d\'expiration', 'show_expiry', !!cfg.show_expiry, setCfg));
      case 'kpi_card':
        return h(React.Fragment, null,
