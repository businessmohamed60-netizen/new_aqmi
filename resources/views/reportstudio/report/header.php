<?php
/** Header — dynamic text with variable substitution. @var array $config @var string $title @var array $template */
$text        = $config['text']  ?? '';
...
// Resolve dynamic variables if template data is available
$tplData    = $template ?? [];
...
<div class="rs-block-header py-2 text-<?= e($align) ?>">
    ...
    <div class="rs-header-meta small text-muted d-flex gap-3 mt-1">
        <?php if ($showReport): ?><span>N° <?= e($rptNumber) ?></span><?php endif; ?>
        <?php if ($showDate): ?><span><?= date('d/m/Y') ?></span><?php endif; ?>
        <?php if ($showPage): ?><span class="rs-dynamic-page">Page {page}/{total_pages}</span><?php endif; ?>
    </div>
    <?php endif; ?>
</div>
